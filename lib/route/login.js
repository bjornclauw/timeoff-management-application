...
module.exports = function(passport) {
  ...
  router.post('/login', function(req, res, next) {
    passport.authenticate('local', function(err, user) {
      ...
      if (!user) {
        req.session.flash_error('Incorrecte gegevens');
      ...
      req.logIn(user, function(err) {
        ...
        req.session.flash_message('Welkom terug '+user.name+'!');
  ...
  router.post('/register', function(req, res){
      ...
      if (!email){
          req.session.flash_error('Email niet ingegeven');
      } else if ( ! validator.isEmail(email)) {
          req.session.flash_error('Email addres ongeldig');
      }

      var name = req.body['name'];
      if (!name){
          req.session.flash_error('Name niet opgegeven');
      }

      var lastname = req.body['lastname'];
      if (!lastname) {
          req.session.flash_error('Achternaam niet opgegeven');
      }
      ...
      .catch(function(error){
          ...
          req.session.flash_error(
            'Nkiet gelukt om nieuwe gebruiker te registreren.'+
  ...
  router.post('/forgot-password/', function(req, res){
    ...
    if (!email){
      req.session.flash_error('Email niet opgegeven');

    } else if ( ! validator.isEmail(email)) {
      req.session.flash_error('Email addres niet geldig');
    }
    ...
    var success_msg ='Controleer uw inbox voor verdere instructies';
  ...
  router.get('/reset-password/', function(req, res){
    ...
    req.app.get('db_model').User.get_user_by_reset_password_token(token)
      .then(function(user){
        if (! user) {
          req.session.flash_error('Link ongeldig, voer uw aanvraag opnieuw uit');
  ...
  router.post('/reset-password/', function(req, res){
    ...
    req.app.get('db_model').User.get_user_by_reset_password_token(token)
      ...
      .then(function(){
        req.session.flash_message('Gebruik uw nieuw wachtwoord om in te loggen');
          return res.redirect_with_session('/login/')
      });
  });
