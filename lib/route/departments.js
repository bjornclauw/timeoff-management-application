...
function generate_all_department_allowances() {
  ...
  var allowance_options = [{ value : 0, caption : 'Geen'}],
    allowance = 0.5;
...
function get_and_validate_department(args) {
  ...
  if (!validator.isFloat(allowance)) {
    req.session.flash_error(
      'Nieuw tegoed voor '+department_name+' moet een cijfer zijn'
    );
  } else if (!((0 <= allowance) && (allowance <= 50))) {
    req.session.flash_error(
      'Nieuw tegoed voor '+department_name+' moet tussen 0.5 en 50 liggen'
    );
  }
  ...
  if (!validator.isNumeric( boss_id ) ) {
    req.session.flash_error(
      'Nieuwe referentie voor beheerder van '+department_name+' moet een cijfer zijn'
    );
  } else if ( ! _.contains(
    ...
    req.session.flash_error(
      'Nieuwe beheerder voor '+department_name+' is niet gekend'
    );
  }
...
router.post('/departments/', function(req, res){
  ...
  .catch(error => {
    ...
    req.session.flash_error(
      'Fout bij aanmaken nieuwe afdeling. Contacteer de beheerder.'
    );
...
router.post('/departments/delete/:department_id/', function(req, res){
  ...
  if (!validator.isInt(department_id)) {
    ...
    req.session.flash_error('Kan afdeling niet verwijderen: verkeerde parameter');
  ...
  .then(function(departments){
    ...
    if (department_to_remove.users.length > 0){
      req.session.flash_error(
        'Kan Afdeling '+department_to_remove.name
          +' niet verwijderen, er is/zijn nog '
          +department_to_remove.users.length+' gebruiker(s)'
      );

      throw new Error('Afdeling heeft nog werknemers');
    }
  ...
  .then(function(){
    req.session.flash_message('Afdeling werd succesvol verwijderd');
    return res.redirect_with_session('/settings/departments/');
  })
...
router.get('/departments/edit/:department_id/', function(req, res){
  ...
  .catch(function(error){
    ...
    req.session.flash_error(
      'Kon gegevens niet ophalen voor deze afdeling'
    );
...
router.post('/departments/edit/:department_id/', function(req, res){
  ...
  .then(function(){
    if (req.body.remove_supervisor_id) {
      ...
      .then(function(){
        req.session.flash_message('Beheerder verwijderd van ' + department.name);
        return Promise.resolve(1);
      });
      ...
      .then(function(){
        req.session.flash_message('Beheerders toegevoegd aan ' + department.name);
        return Promise.resolve(1);
      });
    }
    ...
    .then(function(){
      req.session.flash_message('Afdeling ' + department.name + ' werd bijgewerkt');
      return Promise.resolve(1);
    });
  })
