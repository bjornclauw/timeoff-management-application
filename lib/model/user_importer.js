...
function add_user(args) {
  ...
  if (validated_args.error) {
    ...
    Exception.throw_user_error({
      ...
      user_error : 'Fout bij toevoegen gebruiker',
    });
  }
  ...
  return Promise.resolve()
    ...
    .then(() => Models.Department
      ...
      .then( department => {
        if ( ! department ) {
          Exception.throw_user_error({
            system_error : 'Fout in Bedrijf/afdeling bij aanmaken nieuwe gebruiker '
              + args.department_id + '/' + args.company_id,
            user_error : 'Verkeerde afdeling',
          });
        }
        return Promise.resolve();
      })
...
function add_users_in_bulk(args) {
  ...
  .then(cmp => {
    ...
    if (with_invalid_departments.length > 0) {
      ...
      Exception.throw_user_error({
        user_error : 'Volgende afdelingen niet gevonden in het bedrijf '
