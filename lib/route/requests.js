...
function leave_request_action(args) {
    ...
    .then(function(processed_leave){
      req.session.flash_message('Aanvraag van '+processed_leave.user.full_name()
          +' werd verwerkt');
    ...
    Promise
      ...
      .then(processed_leave => {
        req.session.flash_message(
          'U hebt gekozen om uw reeds bevestigde afwezigheid te annuleren. '
          + (
            processed_leave.is_auto_approve()
            ? ''
            : 'Dit dient goedgekeurd te worden'
          )
        );
