...
class CompanySummary {
  ...
  as_csv_data() {
    ...
    // Put headers
    results.push([
      'Afdeling', 'Naam', 'Voornaam', 'Email',
      'Soort', 'Start', 'Date type', 'Stop', 'Date type'
    ]);
    ...
    self.users
      ...
      .forEach(u => u.my_leaves.forEach(l => {
        ...
        const startDatePart = l.does_start_half_morning() ? 'Voormiddag'
          : l.does_start_half_afternoon() ? 'Namiddag'
            : 'Hele dag';

        const endDatePart = start_date === end_date ? startDatePart
          : l.does_end_half_morning() ? 'Voormiddag'
            : l.does_end_half_afternoon() ? 'AfternNamiddagoon'
              : 'Hele dag';
