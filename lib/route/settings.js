...
router.post('/company/', function(req, res){
  ...
  if (!validator.isAlphanumeric(country_code)){
    req.session.flash_error('Land mag enkel letters en cijfers bevatten');
  }

  if ( ! moment_tz.tz.names().find(tz_str => tz_str === timezone) ) {
    req.session.flash_error('Tijdzone onbekend');
  }

  if ( ! validator.isNumeric(carriedOverDays)) {
    req.session.flash_error('geef aub numerieke waarde op');
  }
  ...
  .then(function(company){
    ...
    if ( _.indexOf( company.get_available_date_formats(), date_format ) < 0 ) {
      var error_msg = 'Onbekend formaat opgegeven';
  ...
  .then(function(){
      req.session.flash_message('Bedrijf werd succesvol aangepast');
      return res.redirect_with_session('/settings/general/');
  })
  .catch(function(error){
    ...
    req.session.flash_error(
      'Fout bij updaten van de gegevens, contacteer de beheerder.'
    );
...
router.post('/carryOverUnusedAllowance/', (req, res) => {
  req.user
    ...
    .then(() => req.session.flash_message(
      'Niet gebruikte tegoeden werden overgezet'
    ))
    .catch(error => {
      ...
      req.session.flash_error(
        `Fout, kan tegoed niet overzetten, contacteer de beheerder en vermeld volgende code: incident ID: ${logMarker}`
      );
    })
  ...
  req.user.getCompany()
    ...
    .then(function(){
      req.session.flash_message( schedule.is_user_specific()
        ? 'Schema voor gebruiker werd opgeslagen'
        : 'Schema voor bedrijf werd opgeslagen'
      );
    })
    ...
    .catch(function(error){
      ...
      req.session.flash_error( schedule.is_user_specific()
        ? 'Fout bij opslaan gebruikersschema'
        : 'Fout bij opslaan bedrijfsschema'
      );
...
router.post('/leavetypes', function(req, res){
  ...
  req.user
    ...
    .then(() => {
      req.session.flash_message('Aanpassingen werden opgeslagen');
      return res.redirect_with_session('/settings/general/');
    })
    .catch(error => {
      console.error(
      ...
      req.session.flash_error(
        'Fout bij aanpassen van de aanvraag, contacteer de beheerder'
      );
...
router.post('/leavetypes/delete/:leave_type_id/', function(req, res){
    ...
    if (!validator.isInt(leave_type_id)) {
        ...
        req.session.flash_error('Kan type niet verwijderen, verkeerde parameter');
    ...
    .then(function(company){
        ...
        // Check if there exist leaves for current type and if so, do not remove it
            ...
            req.session.flash_error('Kan type niet verwijderen, in gebruik');
    ...
    .then(function(){
        req.session.flash_message('Type werd succesvol verwijderd');
        return res.redirect_with_session('/settings/general/');
    })
    .catch(function(error){
        console.error(
...
router.post('/company/integration-api/', (req, res) => {
  ...
  action = action.then(() => {
    req.session.flash_message('Instellingen werden aangepast');
  ...
  action = action.catch(error => {
    ...
    req.session.flash_error(
      `Fout bij aanpassen instellingen. ${ extractUserErrorMessage(error) }`
    );
...
function get_and_validate_leave_type(args) {
  ...
  if ( ! validator.matches(color, /^leave_type_color_\d+$/)) {
    throw_user_error( 'Nieuw kleur voor '+item_name+' geen geldige css klasse' );
  }

  if ( ! validator.isNumeric(limit) ){
    throw_user_error( 'Nieuwe limiet voor '+item_name+' is geen getal' );

  } else if ( limit < 0) {
    throw_user_error( 'Nieuw limiet voor '+item_name+' mag geen negatief getal zijn' );
  }
...
router.get('/company/backup/', (req, res) => {
  ...
  req.user
    ...
    .catch(function(error){
      ...
      req.session.flash_error(
        "Fout bij downloaden bedrijfsgegevens " +
        ( error.show_to_user ? error : 'Please contact customer service')
      );
...
router.post('/company/delete/', (req, res) => {
  ...
  req.user
    ...
    .then(() => {
      req.session.flash_message(`Bedrijf ${company.name} en alle gegevens werden succesvol verwijder`);
    ...
    .catch(error => {
      ...
      req.session.flash_error(
        `Fout bij verwijderen bedrijf. Reden: ${ Exception.extract_user_error_message(error) }`
      );
